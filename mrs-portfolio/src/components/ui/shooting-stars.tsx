...
export const ShootingStars: React.FC<ShootingStarsProps> = ({
  ...
  starColor = "#9E00FF", trailColor = "#2EB9DF", starWidth = 10, starHeight = 1, className,
}) => {
  ...
  useEffect(() => {
    const createStar = () => {
      ...
      const newStar: ShootingStar = {
        ...
        speed: Math.random() * (maxSpeed - minSpeed) + minSpeed, distance: 0,
      };
      setStar(newStar);
  ...
  return (
    <svg ref={svgRef} className={cn("w-full h-full absolute inset-0 pointer-events-none", className)}>
      {star && (
        <rect
          key={star.id} x={star.x} y={star.y} width={starWidth * star.scale} height={starHeight} fill="url(#gradient)"

    ...
    <section className="w-full bg-black py-20 flex flex-col items-center justify-center overflow-hidden border-t border-zinc-800">
      ...
      <div className="relative z-10 text-center mb-16">
        ...
        <p className="text-gray-400 max-w-2xl mx-auto px-4">
          Technologies orbiting around my development universe
        </p>
      </div>
      ...
      >
        ...
        {skillsConfig.map((skill) => (
          <OrbitingSkill
            key={skill.id}
            config={skill}
            angle={skill.phaseShift + time * skill.speed}
          />
        ))}

        {/* Center Core */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 bg-gradient-to-br from-cyan-500/20 to-purple-500/20 rounded-full blur-xl animate-pulse" />
      </div>

      {/* Interaction Hint */}
      <div className="mt-12 text-center text-gray-500 text-sm">
        Hover over skills to pause & explore
      </div>
    </section>
  );
}
